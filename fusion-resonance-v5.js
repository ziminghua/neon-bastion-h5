(() => {
  ...
  function relation(left,right,leftPosition,rightPosition){
    if(left.type===right.type) return {active:false,distance:Infinity,radius:0};
    const radius=Math.min(RADII[left.type]||0,RADII[right.type]||0);
    const value=distance(leftPosition,rightPosition);
    return {active:value<=radius,distance:value,radius};
  }
  ...
  function buildNetwork(now=performance.now()){
    ...
    for(let left=0;left<nodes.length-1;left+=1){
      for(let right=left+1;right<nodes.length;right+=1){
        const edge=relation(nodes[left].tower,nodes[right].tower,nodes[left].position,nodes[right].position);
        if(!edge.active) continue;
        const link={
          from:nodes[left].tower,
          to:nodes[right].tower,
          distance:edge.distance,
          radius:edge.radius,
          combo:pairKey(nodes[left].tower.type,nodes[right].tower.type)
        };
        nextLinks.push(link);
        nextPartners.get(link.from).push(link.to);
        nextPartners.get(link.to).push(link.from);
      }
    }
    ...
    for(const tower of towers){
      ...
      while(queue.length){
        ...
        for(const partner of nextPartners.get(current)||[]){
  ...
  function baseDef(tower){
    ...
    const source=tower.__crossTypeBaseDef||tower.def;
  ...
  function updateBadges(){
    ...
    for(const [type,id] of Object.entries(badgeByType)){
      ...
      badge?.setAttribute('title',active?'Fusion ability active with every different tower in range':'Place a different tower type inside fusion range');
    }
  }
  ...
  function drawGuide(now){
    ...
    if(guide.mode==='placement'){
      const candidates=game.state.towers
        ...
        .map(item=>({...item,edge:relation({type:guide.type},item.tower,guide.position,item.position)}))
        .filter(item=>item.edge.active);
  ...
  function publish(guide){
    ...
    window.__COMBAT_BALANCE_DIAGNOSTICS={
      ...
      resonancePolicy:'every different tower pair inside mutual range forms its own fusion link; fusion changes attack behavior instead of granting generic damage',
      links:links.map(link=>({
        fromType:link.from.type,toType:link.to.type,fromSlot:link.from.slot,toSlot:link.to.slot,
        distance:link.distance,radius:link.radius,combo:link.combo,comboName:COMBOS[link.combo]
      })),
      ...
      guide:guide?{type:guide.type,mode:guide.mode,position:guide.position?{...guide.position}:null,radius:RADII[guide.type]}:null,
      fusionChannels:{projectiles:projectileChannelInstalled,beams:beamChannelInstalled},
      overlayReady:Boolean(overlay)
    };
    window.__FUSION_RESONANCE_RUNTIME={build:BUILD,ready:true,linkCount:links.length,combos:{...COMBOS}};
  }
