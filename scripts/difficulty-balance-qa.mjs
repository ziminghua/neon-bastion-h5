...
// Reset safety: enemy IDs restart from 1, but fresh enemy objects must still receive scaling.
await page.evaluate(()=>{
  const game=window.__NEON_TEST__;
  game.resetGame();
  game.state.wave=1;
...
const resetProbe=await page.evaluate(()=>{
  const enemy=window.__NEON_TEST__.state.enemies[0];
  return {id:enemy.id,maxHp:enemy.maxHp,multiplier:enemy.__difficultyMultiplier,build:enemy.__difficultyBuild};
});
...
// Early-game fairness: two affordable towers should still survive wave 1.
await page.evaluate(()=>{
  const game=window.__NEON_TEST__;
  game.resetGame();
...
await page.waitForFunction(()=>!window.__NEON_TEST__.state.waveActive||window.__NEON_TEST__.state.hp<=0,null,{timeout:60000});
...
if(lateRun.hp<=0||!lateRun.resultVisible||!lateRun.title.includes('SECURED')) errors.push(`wave 5 overtuned against a full mixed board: ${JSON.stringify(lateRun)}`);

const report={errors,runtime,samples,resetProbe,earlyRun,lateRun};
