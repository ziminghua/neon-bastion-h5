...
const LEVEL = {
  ...
  slots: [
    ...
    {x:505,y:470,zone:'control'},{x:390,y:165,zone:'control'},
...
function drawTower(t){
  const home=LEVEL.slots[t.slot], pos=towerDrawPosition(t), def=t.def, baseScale=(.265+.016*(t.level-1))*(1+(t.mergePulse||0)*.11), recoil=t.recoil*8;
  if(pos.dragging){
...
function drawEnemy(e){
  const p=pathPoint(e.progress), bob=Math.sin(e.bob)*3, scale=(e.def.boss?.27:e.type==='brute'||e.type==='shield'?.22:e.type==='runner'?.19:.18)*e.spawnScale;
